...
(function () {
    ...
    try {
        ...
        document.addEventListener("keypress", function (evt: KeyboardEvent) {
    ...
    function ShowSpeedChange(vid: HTMLVideoElement) {
        const rect = vid.getBoundingClientRect();
        const txtID = `_speedText_${Date.parse(new Date().toUTCString())}${Math.random()}`;
        ...
        setTimeout(function () {
            ...
            doc !== null ? doc.remove() : document.querySelectorAll("[id^='_speedText_']").forEach((e) => e.remove());
        }, 250);
    }
})();
